...
(defn boundary->polygon
  ...
  (try
    (let [;; Filter out nil coordinates and ensure doubles
          ...
          closed-coords (if (and (seq vector-coords)
                                (= (first vector-coords) (last vector-coords)))
                         vector-coords
                         (conj (vec vector-coords) (first vector-coords)))]
      (when (>= (count closed-coords) 4) ; Minimum for a valid polygon
        (let [ring (jts/linear-ring closed-coords)
...
(defn point-in-borough?
  ...
  (try
    (spatial/intersects? borough-polygon (jts/point lng lat))
    (catch Exception e
      false)))
